...
public class GLSLProgram {
	...
	public void draw(GameObject vertices, int mode, int count) {
		 ...
		 * to render. Symbolic constants GL_POINTS, GL_LINE_STRIP, GL_LINE_LOOP,
		 * GL_LINES, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN, and GL_TRIANGLES are
		 * accepted. GLsizei count, Specifies the number of elements to be
		 * rendered. GLenum type, Specifies the type of the values in indices.
		 * Must be GL_UNSIGNED_BYTE or GL_UNSIGNED_SHORT. const GLvoid * indices
		 * Specifies a pointer to the location where the indices are stored. );
		 * 
		 * Prototype de la fonction Description
		 * 
		 * 
		 * on Crée une géométrie avec le type de primitive "mode", cette
		 * géométrie est composée de n vertex les vertex à utiliser sont
		 * spécifié dans le tableau des indices
		 * 
		 * par exemple : on veut dessiner un carré, pour cela on va faire 2
		 * triangles côte à côte on utilise le mode GL_TRIANGLES pour dessiner
		 * un triangle, il faut 3 sommets, on demande donc a dessiner la forme
		 * avec les groupe de 3 vertex, on va donc lire notre tableau des
		 * indices par groupe de 3. pour savoir quels vertex sont à utiliser, on
		 * récupère les n° de vertex référencé dans le tableau des indices (pour
   ...
   * void glVertexAttribPointer(	GLuint index,		-Specifies the index of the generic vertex attribute to be modified.
	...
	private boolean link() {
		...
		GLES20.glGetProgramiv(mProgramObject, GLES20.GL_LINK_STATUS,
				linkStatus, 0);
		if (linkStatus[0] != GLES20.GL_TRUE) {
			Log.e(this.getClass().getName(), "Could not link program: ");
			Log.e(this.getClass().getName(),
					"logs:" + GLES20.glGetProgramInfoLog(mProgramObject));
			GLES20.glDeleteProgram(mProgramObject);
			mProgramObject = 0;
			return false;
		}
	...
	private boolean loadShaders(String vertexFilename, String fragmentFilename) {
		...
		mFragmentShader = loadShader(fragmentFilename,
