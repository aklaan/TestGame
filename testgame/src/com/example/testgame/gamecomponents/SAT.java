...
public class SAT {
	...
	public static boolean isColide(GameObject shape1, GameObject shape2) {
		...
		for (int i = 0; i < shape1_Normals.size(); i++) {
			...
			// c'est qu'il existe un écart entre le point le plus à droite de la shape 1
			// et le point le plus à gauche de la shape2
			// on test l'inverse aussi.
			isSperarated = shape1_maxproj < shape2_minproj
					|| shape2_maxproj < shape1_minproj;
			
			// dès que l'on détécte le moindre écart c'est qu'il n'ya pas colission
			...
			// on a pas décelé d'écart entre la shape1 et shape2 avec en utilisant les normale
			// de la shape1. on vérifie si c'est toujours le cas avec les normales de la shape2
			for (int j = 0; j < shape2_Normals.size(); j++) {
				...
				isSperarated = shape1_maxproj < shape2_minproj
						|| shape2_maxproj < shape1_minproj;
				// si a ce stade on détecte une séparation alors la règle
				// stipulant
				// que toutes
		...
		if (isSperarated) {
			// Log.i("séparé?", String.valueOf(isSperarated));
		}
